...
public class Query implements SQLProducer, Serializable {
	...
	private String queryModality() {
		...
		// NestedFilter.
		if (filter instanceof NestedFilter && !((NestedFilter) filter).isEmpty()) {
		...
		String sql = "";
		sql += "SELECT DISTINCT " + getColumnString(); //DISTINCT
		...
		if (!NoFilter.getInstance().equals(filter) && filter instanceof NestedFilter && !((NestedFilter) filter).isEmpty() && filter.toSQLString() != null) {
			if (clause.length() > 0) {
				sql += " WHERE ";
			}
		...
		//Window.alert("An sql string was generated: \"" + sql + "\"");
		// For debugging.
		//sql +=  " LIMIT 500";
	...
	protected String getColumnString() {
		...
		if (!columns.isEmpty()) {
			...
			while (iter.hasNext()) {
				...
				if(col instanceof IntervalTableColumn){
					IntervalTableColumnFilter itf = (IntervalTableColumnFilter) ((NestedFilter)filter).findFilter(col);
					tmp = ((IntervalTableColumn)col).toSQLString(itf.getTimeInterval());
					tmp = tmp.concat(" AS \"" + ((IntervalTableColumn)col).getName() + "\"");
				}
				else
					tmp = col.toSQLString();
	...
	public String getJoinedTables() {
		...
		ArrayList<Column> flat_columns = new ArrayList<Column>();
		for(int i=0;i<columns.size();i++){
			if(!(columns.get(i) instanceof IntervalTableColumn))
				flat_columns.add(columns.get(i));
			else{
				flat_columns.add(((IntervalTableColumn)columns.get(i)).getStartColumn());
				flat_columns.add(((IntervalTableColumn)columns.get(i)).getEndColumn());
			}
		}
		
		// Grab columns appearing in the filter (Ie the WHERE part of the clause). These might need to be joined as well if they dont appear in the SELECT part.
		if (!NoFilter.getInstance().equals(filter) && filter instanceof NestedFilter && !((NestedFilter) filter).isEmpty() && filter.toSQLString() != null) {
			Set<Column> cur_filter_columns = ((NestedFilter) filter).getColumns();
			for (Column c: cur_filter_columns){
				if(!(c instanceof IntervalTableColumn))
					flat_columns.add(c);
				else{
					flat_columns.add(((IntervalTableColumn)c).getStartColumn());
					flat_columns.add(((IntervalTableColumn)c).getEndColumn());
				}
			}
		}

		ListIterator<Column> iter = flat_columns.listIterator();
		String colName;
		...
		while ((colName = getNextTableName(iter)) != null) {
			...
			if (!colName.startsWith("simquery") && !colName.startsWith("SKIPME") && !ret.contains(colName)) {
