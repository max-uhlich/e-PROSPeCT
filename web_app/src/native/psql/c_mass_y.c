...
/* Returns the <YVALUE>, expected to be a float literal, from a string in the
 * format:
 *
 ...
 * not able to extract the YVALUE, return -1  (all valid values are expected
 * to be non-negative).
 */
/* PG_FUNCTION_INFO_V1(c_mass_y); */
PG_FUNCTION_INFO_V1(c_mass_y);
float4* c_mass_y(PG_FUNCTION_ARGS) {
	...
	float x_dummy;
	float4 y;
	int found = sscanf(null_term_input, "CMx=%f;CMy=%f;", &x_dummy, &y);
